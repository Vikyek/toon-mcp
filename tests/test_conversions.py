...
class TestTOONConverter:
    ...
    def test_aggressive_mode_conversion(self):
        ...
        # Aggressive mode should emit a _custom_keys mapping
        assert '_custom_keys' in toon_obj
        custom_keys = toon_obj['_custom_keys']
        assert len(custom_keys) > 0
        # Domain keys like 'product_id', 'product_name' should be in the map
        assert any('product' in k for k in custom_keys)
